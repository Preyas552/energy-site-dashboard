...
export async function POST(request: NextRequest) {
  try {
    ...
    // Extract most_likely values from fuzzy site data to create crisp alternatives
    ...
    // Extract most_likely values from fuzzy weights to create crisp weights
    const crispWeights = [
    ...
    console.log('Sending to Python crisp TOPSIS service:', PYTHON_SERVICE_URL);
    console.log('Number of alternatives:', alternatives.length);
    console.log('Sample alternative:', alternatives[0]);
    console.log('Crisp weights:', crispWeights);
    ...
    console.log('Python crisp TOPSIS response:', result);
    ...
    return NextResponse.json(
      {
        success: false,
        error: error.message || 'Failed to analyze sites with crisp TOPSIS',
      },
      { status: 500 }
    );
  }
